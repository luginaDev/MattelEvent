...
class Members extends Backend_Controller {
  function __construct()
    ...
    public function save(){
      ...
      if(!empty($data)){
        ...
        if($data["type_proceses"]=="update"){
          if($this->m->update_attribute(array("no_ktp"=> $data["no_ktp"]),$member)){
            $this->session->set_flashdata('success', 'Data member berhasil diupdate');
          }else{
            $this->session->set_flashdata('error', 'Data member Gagal diupdate');
          }
        }else{
  ...
  public function ajax_show(){
    ...
    $data = $this->m->find_entity_by("no_ktp",$id);
