<script>
  $(document).ready(function(){
    $(".curd_edit").click(function(){
      ...
      var kecamatan = $("#kecamatan_id-"+id).val();
      $("#frm-id").val(id);
      $("#frm-nama").val(nama);
      $('#myModal').modal('show');
      ...
      if(kecamatan!=""){
        $.ajax({
          ...
          data :"id="+kota+"&kecamatan="+kecamatan ,
          type :"POST",
          success : function(data){
    ...
    <?php foreach($data[0] as $i => $d): ?>
      <tr>
        ...
        <td><?=$d->kecamatan?><input type="hidden" id="kecamatan_id-<?=$d->id?>" value="<?=$d->kecamatan_id?>"/></td>
        <td>
          <a href="#" class="curd_edit" id="<?=$d->id?>"> Edit</a> | 
          <a href="<?=base_url("kelurahan/delete/$d->id")?>" > Hapus </a> 
        </td>
      </tr>
    <?php endforeach;?>
    </tbody>
...
<form class="form-horizontal" method="post" action="<?=base_url("kelurahan/save")?>">
